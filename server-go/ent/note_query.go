...
func (_q *NoteQuery) QueryTenant() *TenantQuery {
...
func (_q *NoteQuery) QueryVault() *VaultQuery {
...
func (_q *NoteQuery) WithTenant(opts ...func(*TenantQuery)) *NoteQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *NoteQuery) WithVault(opts ...func(*VaultQuery)) *NoteQuery {
	query := (&VaultClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Note, error) {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *Note, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withVault; query != nil {
		if err := _q.loadVault(ctx, query, nodes, nil,
			func(n *Note, e *Vault) { n.Edges.Vault = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NoteQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Note, init func(*Note), assign func(*Note, *Tenant)) error {
...
func (_q *NoteQuery) loadVault(ctx context.Context, query *VaultQuery, nodes []*Note, init func(*Note), assign func(*Note, *Vault)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Note)
	for i := range nodes {
		fk := nodes[i].VaultID
...
func (_q *NoteQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTenant != nil {
			_spec.Node.AddColumnOnce(note.FieldTenantID)
		}
		if _q.withVault != nil {
			_spec.Node.AddColumnOnce(note.FieldVaultID)
		}
	}
