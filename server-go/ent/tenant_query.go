...
func (_q *TenantQuery) QueryVaults() *VaultQuery {
...
func (_q *TenantQuery) WithVaults(opts ...func(*VaultQuery)) *TenantQuery {
	query := (&VaultClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withVaults; query != nil {
		if err := _q.loadVaults(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Vaults = []*Vault{} },
			func(n *Tenant, e *Vault) { n.Edges.Vaults = append(n.Edges.Vaults, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TenantQuery) loadVaults(ctx context.Context, query *VaultQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Vault)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(vault.FieldTenantID)
	}
	query.Where(predicate.Vault(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.VaultsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TenantQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Asset)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(asset.FieldTenantID)
	}
