...
// The NoteAssetRefFunc type is an adapter to allow the use of ordinary
// function as NoteAssetRef mutator.
type NoteAssetRefFunc func(context.Context, *ent.NoteAssetRefMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f NoteAssetRefFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.NoteAssetRefMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.NoteAssetRefMutation", m)
}
